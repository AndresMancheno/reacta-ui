...
- `packages/@andresmancheno/reacta-ui`: Core React components
- `packages/@ui/utils`: Shared React utilities
- `packages/@ui/tsconfig`: Shared `tsconfig.json`s used throughout the Turborepo
- `packages/ui-eslint-config`: ESLint preset
...
Turborepo runs the `build` script for all publishable packages (excluding docs) and publishes the packages to npm. By default, this example includes `ui` as the npm organization. To change this, do the following:

- Rename folders in `packages/*` to replace `ui` with your desired scope
- Search and replace `ui` with your desired scope
- Re-run `pnpm install`
